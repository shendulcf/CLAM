...
from utils.utils_myself import *
import os
...
def train(datasets, cur, args):
    ...
    if args.model_type in ['clam_sb', 'clam_mb', 'transmil', 'mcbat_sb']: # ---->> 1
        if args.subtyping:
        ...
        elif args.model_type == 'mcbat_sb': # ---->> 2
            model = MCBAT_SB(**model_dict, instance_loss_fn=instance_loss_fn)
        else:
    ...
    for epoch in range(args.max_epochs):
        ...
        if args.model_type in ['clam_sb', 'clam_mb','transmil','mcbat_sb'] and not args.no_inst_cluster:  # ---->> 3   
            ...
            stop = validate_mcbat(cur, epoch, model, val_loader, args.n_classes, 
...
def train_loop_mcbat(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None):
    ...
    for batch_idx, (data, data2, label) in enumerate(loader):
        ...
        logits, Y_prob, Y_hat, _, instance_dict = model(data, data2,label=label, instance_eval=True)
...
def validate_mcbat(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    inst_logger = Accuracy_Logger(n_classes=n_classes)
...
def summary(model, loader, n_classes):
    ...
    for batch_idx, (data,data2, label) in enumerate(loader):
        ...
        with torch.no_grad():
            logits, Y_prob, Y_hat, _, _ = model(data, data2)
